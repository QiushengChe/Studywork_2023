...
plt.plot(range(1, itex_num + 1), acc_t5_list, marker='*', label='Top-5 accuracy + wo augm')
plt.plot(range(1, itex_num_augm + 1), acc_t1_list_augm, marker='*', label='Top-1 accuracy + w augm')
plt.plot(range(1, itex_num_augm + 1), acc_t5_list_augm, marker='*', label='Top-5 accuracy + w augm')
plt.xlim((0, max(itex_num, itex_num_augm)))
...
print("t1:", sum(time_list), "t2:", sum(time_list_augm))
print("T1 delta:", (acc_t1_list_augm[-1] - acc_t1_list[-1]), "T5 delta:", (acc_t5_list_augm[-1] - acc_t5_list[-1]))
plt.show()
